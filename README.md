...
<details>
<summary>ブラウザの起動オプションについて</summary>

`login` では、起動したブラウザに
`--disable-blink-features=AutomationControlled` を指定しています。

これを付けないと、Google のログイン画面が
「このブラウザまたはアプリは安全でない可能性があります」として
ログインを拒否し、**自分のアカウントにログインできません**。

ログイン操作そのものは利用者ご自身が手動で行います。
このツールが ID やパスワードを入力・保存することはありません。
取得するのは、ログイン後に発行される Cookie のみです。

この挙動を避けたい場合は `--manual` をご利用ください。
